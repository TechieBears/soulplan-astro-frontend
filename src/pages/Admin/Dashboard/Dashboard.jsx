...
const Dashboard = () => {
  ...
  const apiCall = async () => {
    try {
      getDashboardInsights(user?.role == "admin" ? "" : user?._id).then(
        (res) => {
          setInsites(res?.data);
        }
      );
        ...
        <div className="flex items-center gap-3 py-2">
          ...
          <div>
            <div className="font-semibold text-gray-900 text-base">
              {row?.cust
                ? `${row.cust.firstName} ${row.cust.lastName}`
                : "------- -------"}
            </div>
        ...
        <div className="py-2">
          ...
          <div className="flex items-center gap-1.5 text-xs text-gray-600">
            ...
            <span className="font-medium">
              {row?.snapshot?.durationInMinutes || 0} minutes
            </span>
          </div>
        </div>
      ),
      ...
      body: (row) => (
        <div className="py-2">
          ...
          <div className="mt-1.5 text-xs text-gray-600 font-medium max-w-xs truncate">
            {row?.items?.map((item) => item.snapshot.name).join(", ") ||
              "No items"}
          </div>
        </div>
      ),
  ...
  return (
    <>
      <div className="px-1 mx-4 m-2 rounded-xl pt-4">
        <h1 className="text-2xl font-bold font-tb text-black  px-2 mx-4">Service Analytics</h1>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pt-2 pb-4">
          {/* Total Bookings Card */}
          <div className="bg-white p-5 rounded-3xl space-y-1.5 relative overflow-hidden">
            ...
            <h6 className="text-slate-500 font-tb font-normal text-xs">
              All time bookings
            </h6>
          </div>
      ...
      {user?.role == "admin" && (
        <div className="px-1 mx-4 m-2 rounded-xl">
          <h1 className="text-2xl font-bold font-tb text-black px-2 pt-4 mx-4">Products Analytics</h1>
          <div className=" grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pt-2 pb-4">
            <div className="bg-white p-5 rounded-3xl space-y-1.5 relative overflow-hidden">
              <div className="absolute -top-8 -right-8 w-[7.7rem] h-[7.7rem] bg-purple-300  rounded-[40px] flex items-center justify-center" />
              <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                ...
                <h6 className="text-slate-500 font-tb font-normal text-sm">
                  total product sell
                </h6>
              </div>
            </div>
      ...
      <div className="bg-white rounded-xl m-4 sm:m-5 shadow-sm p-5 sm:p-7">
        <TableHeader
          ...
          subtitle={`${
            insites?.todaysBookings?.length || 0
