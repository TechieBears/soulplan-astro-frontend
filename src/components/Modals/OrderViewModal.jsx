                                ...
                                <div className="flex items-center justify-between border-b bg-gray-50 sticky top-0 z-10 p-4 sm:px-6">
                                    ...
                                    <div className="flex items-center gap-x-4">
                                        {orderType === "product" ? (
                                            ...
                                            <>
                                                <button className={`flex items-center space-x-2 sm:p-2 text-md sm:text-md font-tbLex capitalize font-medium rounded ${getOrderStatus() === 'confirmed' ? 'bg-green-50 text-green-500' :
                                                    getOrderStatus() === 'pending' ? 'bg-yellow-50 text-yellow-500' :
                                                        'bg-green-50 text-green-500'
                                                    }`}>
                                                    <img src={verified} alt="" className="w-6 h-6" />
                                                    <span className="hidden sm:inline">Status: {getOrderStatus()}</span>
                                                </button>
                                                <button className={`flex items-center space-x-2 sm:p-2 text-md sm:text-md font-tbLex capitalize font-medium rounded ${getPaymentStatus() === 'paid' ? 'bg-green-50 text-green-500' :
                                    ...
                                    <div className="space-y-6">
                                        {orderType === "product" ? (
                                            ...
                                            orderData.items?.map((item, itemIndex) =>
                                                <RenderProductView key={itemIndex} item={item} itemIndex={itemIndex} orderData={orderData} orderType={orderType} />
                                            )
                                        ) : (
                                            // Render service items
                                            orderData.services?.map((service, serviceIndex) =>
                                                <RenderServiceView key={serviceIndex} service={service} serviceIndex={serviceIndex} orderData={orderData} orderType={orderType} />
                                            )
                                        )}
...
const RenderProductView = ({ item, itemIndex, orderData }) => {
    ...
    const { handleSubmit, reset, control } = useForm({ defaultValues: { rating: 0, reviewText: "" } });
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-3">
                        ...
                        <form onSubmit={handleSubmit(handleAddRating)}>
                            <div className="mt-3 flex flex-col gap-3 bg-slate-50 rounded-lg p-4">
                                ...
                                <div className="flex justify-end gap-2">
                                    <button
                                        type="button"
                                        onClick={() => { setActiveReview(!activeReview); reset() }}
                    ...
                    <div className="mt-4 space-y-4">
                        <h5 className="font-semibold text-gray-800 font-tbPop">Product Reviews</h5>
                        <div className="overflow-y-auto max-h-[300px] space-y-4">
                            {ratingsLoading ? <div className="flex justify-center items-center h-20"> Loading<PulseLoader color="#000" size={4} /></div> :
                                ratings?.length > 0 ?
                                    <>
                                        {ratings?.map((review, i) => (
                                            <div key={i} className="flex gap-3 bg-gray-50 p-3 rounded-lg">
                                                <img
                                                    src={review.user?.profileImage || "https://i.pravatar.cc/40?img=3"}
                                                    alt="avatar"
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <p className="text-gray-700 text-sm font-tbPop">{review.message}</p>
                                                </div>
                                            </div>
